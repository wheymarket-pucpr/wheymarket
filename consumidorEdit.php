...
if (!empty($_GET['id'])) {
    $id = $_GET['id'];
    $sqlSelect = "SELECT * FROM consumidor WHERE ID=$id";
    $result = $conn->query($sqlSelect);
    if ($result->num_rows > 0) {
        $consumidor = mysqli_fetch_assoc($result);
...
<body>
    ...
    <div class='.container-fluid'>
        <div class="row justify-content-center pt-5">
            <div class='col-3'>
                <div class="card">
                    <h5 class="card-header text-dark">Editar cadastro</h5>
                    <div class="card-body">
                        <div class='col'>
                            <form action="consumidorSaveEdit.php" method="POST" class="needs-validation" novalidate>
                                <div class="mb-3">
                                    <label for="email" class="form-label">Email</label>
                                    <input type="email" name="email" class="form-control" id="email" placeholder="example@example.com" 
                                    value="<?php  echo $consumidor['email']?>" maxlength="50" required />
                                ...
                                <div class="mb-3">
                                    <label for="CPF" class="form-label">CPF <span style="color: red">(Nao editável)</span></label>
                                    <input type="text" name="CPF" class="form-control CPF" id="CPF" maxlength="14"  pattern="^(?=.*[A-Z])(?=.*[0-9])(?=\S{8,20}$).*"
                                     readonly value="<?php  echo $consumidor['CPF']?>" />
                                    <div class="invalid-feedback" >
                                ...
                                <div class="mb-3">
                                    ...
                                    value="<?php echo  $consumidor['Nome']?>" maxlength="100" required />
                                    <div class="invalid-feedback">
                                ...
                                <div class="d-grid gap-2 mb-3 mt-3">
                                    <input name="ID" type="hidden" value="<?php echo $consumidor['id']?>"> 
